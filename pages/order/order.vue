					...
					<view v-for="(item,index) in tabItem.orderList" :key="index" class="order-item">
						...
						<scroll-view v-if="item.goodsList.length > 1" class="goods-box" scroll-x>
							<view v-for="(goodsItem, goodsIndex) in item.goodsList" :key="goodsIndex"
								class="goods-item">
								<image class="goods-img" :src="goodsItem.preview" mode="aspectFill"></image>
							</view>
						</scroll-view>
						<view v-if="item.goodsList.length === 1" class="goods-box-single"
							...
							<view class="right">
								...
								<text class="attr-box" v-if="goodsItem.attrs!=='-'">{{goodsItem.attrs}} x
									{{goodsItem.number}}</text>
								<text class="attr-box" v-else>{{goodsItem.number}}</text>
	...
	export default {
		...
		methods: {
			...
			loadData(source) {
				...
				setTimeout(() => {
					let url = "/user/orders?state=" + state
					if (navItem.orderList.length > 0) {
						url += "&next=" + navItem.orderList[navItem.orderList.length - 1].id
					}
					$http.request({
						url: url,
						token: true,
						...
						value.forEach(item => {
							item.goodsList.forEach(val => {
								val.preview = $http.media(val.preview)
							})
			...
			cancelOrder(item) {
				$http.request({
					url: "/user/order?order_id=" + item.id,
					method: "DELETE",
					token: true
				}).then(() => {
					let {
